Feature: Verify All Clubs , ACTIVE COMPLIMENTARY Member , MPQ - 1, Unlimited Redemption,2$ Off each for Emulator Mercury: Buy X get $Y off on X

  Scenario Outline: Emulator QuickSilver Promos for Regular Mix and Match Single Order Lines for Promo Item 622943 with ACTIVE COMPLIMENTARY Member
    Given Minimum <Quantity> required for Emulator with <Channel>,<ItemId>,<ClubId> and <ChannelCode> with <RetailPrice> and Single OrderLine <OrderLines> in <Applied_Dates> to be Utilised to Get Offer <Offer Id> for complimentary member
    When Post request to Emulator Endpoint for Complimentary Member
    When Request posted to Emulator and get the required discount for Complimentary Member
    Then Assert required to Emulator and get attributes for Complimentary Member
  ...
  Scenario Outline: Emulator QuickSilver Promos for Regular Mix and Match Double Order Lines for Promo Item 622943 and 984102  with ACTIVE COMPLIMENTARY Member
    Given Minimum <Quantity> required for Emulator with <Channel>,<ItemId>,<ClubId> and <ChannelCode> with <RetailPrice> and Single OrderLine <OrderLines> in <Applied_Dates> to be Utilised to Get Offer <Offer Id>
    Given Minimum <Quantity2> required for Emulator with Second <ItemId2> and <ClubId> with <RetailPrice> and Second OrderLine <SecondOrderLine> to be Utilised to Get Offer for Complimentary Member
    When Post request to Emulator Endpoint for Complimentary Member
    When Request posted to Emulator and get the required discount for Complimentary Member
    Then Assert required to Emulator and get attributes for Complimentary Member
      | Offer Description   | GS1Code   | Offer Id   | Gtin   | Discount   |
