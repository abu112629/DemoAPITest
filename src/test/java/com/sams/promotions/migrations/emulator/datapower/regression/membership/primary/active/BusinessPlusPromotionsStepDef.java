...
public class BusinessPlusPromotionsStepDef extends BaseStep {
	...
	public BusinessPlusPromotionsStepDef() throws IOException {
		super();
	}
	...
	@Given("^Minimum required for DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) in (.*) to be Utilised to Get Offer for Business Primary Plus Member$")
	public void createtheRequestBusinessPlus(String channelName, String ClubId, String ClubId2, int code,
		...
		while (i < size) {
			...
			POST_the_requestBusinessPlus();
			Getparameter_and_postOperationBusinessPlus();

			POST_the_emulator_requestBusinessPlus();
			Getemulatorparameter_and_postOperationBusinessPlus();

			ChecktheResultsBusinessPlus();
	...
	@When("^Post request to DataPower Endpoint for Business Primary Plus Member$")
	public void POST_the_requestBusinessPlus() {
	...
	@When("^Post request to Emulator Endpoint for Business Primary Plus Member$")
	public void POST_the_emulator_requestBusinessPlus() {
	...
	@When("^Request posted to DataPower and get the required discount for Business Primary Plus Member$")
	public void Getparameter_and_postOperationBusinessPlus() {

		response = thisRequestSpecification.post(UrlConstants.SERVICES_CHECKOUT).then();

	}

	@When("^Request posted to Emulator and get the required discount for Business Primary Plus Member$")
	public void Getemulatorparameter_and_postOperationBusinessPlus() {

		response2 = thisRequestSpecification2.post(UrlConstants.RESERVE_EMULATION).then();

	}

	@Then("^Assert required to DataPower and get attributes for Business Primary Plus Member$")
	public void ChecktheResultsBusinessPlus() throws Exception {
