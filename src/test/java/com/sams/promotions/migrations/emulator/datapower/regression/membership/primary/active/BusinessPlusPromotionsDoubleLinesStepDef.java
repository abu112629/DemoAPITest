...
public class BusinessPlusPromotionsDoubleLinesStepDef extends BaseStep {
	...
	public BusinessPlusPromotionsDoubleLinesStepDef() throws IOException {
		super();
	}
	...
	@Given("^DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) and Double Order Line (\\d+) in (.*) to be Utilised to Get Offer for Business Plus Primary Member$")
	public void createthesingleLineRequestBusinessPlus(String channelName, String ClubId, String ClubId2, int code,
		...
		while (i < size) {
			...
			UpdatetheRequesttoDoubleLinesBusinessPlus(ClubId, ClubId2, RetailPrice, lineNumber2, Applied_Dates);

			POST_the_request_for_Double_linesBusinessPlus();
			Getparameter_and_postOperation_for_Double_LinesBusinessPlus();

			POST_the_emulator_request_for_Double_linesBusinessPlus();
			Getemulatorparameter_and_postOperation_for_Double_linesBusinessPlus();

			ChecktheResultsdoubleLinesBusinessPlus();
	...
	@Given("^DataPower with Second Item and (\\d+),(\\d+) with (\\d+) and Second OrderLine (\\d+) in (.*) to be Utilised to Get Offer for Business Plus Primary Member$")

	public void UpdatetheRequesttoDoubleLinesBusinessPlus(String ClubId, String ClubId2, int RetailPrice,
	...
	@When("^Response for DataPower and get the required discount for Business Plus Primary Member$")
	public void Getparameter_and_postOperation_for_Double_LinesBusinessPlus() {

		response = thisRequestSpecification.post(UrlConstants.SERVICES_CHECKOUT).then();

	}

	@When("^Response for Emulator and get the required discount for Business Plus Primary Member$")
	public void Getemulatorparameter_and_postOperation_for_Double_linesBusinessPlus() {

		response2 = thisRequestSpecification2.post(UrlConstants.RESERVE_EMULATION).then();

	}

	@Then("^Assert on DataPower request and get attributes for Business Plus Primary Member$")
	public void ChecktheResultsdoubleLinesBusinessPlus() throws Exception {
