...
public class SavingsPlusAddOnPromotionsDoubleLinesStepDef extends BaseStep {
	...
	public SavingsPlusAddOnPromotionsDoubleLinesStepDef() throws IOException {
		super();
	}
	...
	@Given("^DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) and Double Order Line (\\d+) in (.*) to be Utilised to Get Offer for Savings Plus AddOn Member$")
	public void createthesingleLineRequestSavingsPlusAddOn(String channelName, String ClubId, String ClubId2, int code,
		...
		while (i < size) {
			...
			UpdatetheRequesttoDoubleLinesSavingsPlusAddOn(ClubId, ClubId2, RetailPrice, lineNumber2, Applied_Dates);

			POST_the_request_for_Double_linesSavingsPlusAddOn();
			Getparameter_and_postOperation_for_Double_LinesSavingsPlusAddOn();

			POST_the_emulator_request_for_Double_linesSavingsPlusAddOn();
			Getemulatorparameter_and_postOperation_for_Double_linesSavingsPlusAddOn();

			ChecktheResultsdoubleLinesSavingsPlusAddOn();
	...
	@Given("^DataPower with Second Item and (\\d+),(\\d+) with (\\d+) and Second OrderLine (\\d+) in (.*) to be Utilised to Get Offer for Savings Plus AddOn Member$")

	public void UpdatetheRequesttoDoubleLinesSavingsPlusAddOn(String ClubId, String ClubId2, int RetailPrice,
	...
	@When("^DataPower Endpoint POST request for Savings Plus AddOn Member$")
	public void POST_the_request_for_Double_linesSavingsPlusAddOn() {
	...
	@When("^Post the request to Emulator Endpoint for Savings Plus AddOn Member$")
	public void POST_the_emulator_request_for_Double_linesSavingsPlusAddOn() {
	...
	@When("^Response for DataPower and get the required discount for Savings Plus AddOn Member$")
	public void Getparameter_and_postOperation_for_Double_LinesSavingsPlusAddOn() {

		response = thisRequestSpecification.post(UrlConstants.SERVICES_CHECKOUT).then();

	}

	@When("^Response for Emulator and get the required discount for Savings Plus AddOn Member$")
	public void Getemulatorparameter_and_postOperation_for_Double_linesSavingsPlusAddOn() {

		response2 = thisRequestSpecification2.post(UrlConstants.RESERVE_EMULATION).then();

	}

	@Then("^Assert on DataPower request and get attributes for Savings Plus AddOn Member$")
	public void ChecktheResultsdoubleLinesSavingsPlusAddOn() throws Exception {
