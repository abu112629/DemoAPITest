...
public class BusinessBaseAddOnPromotionsDoubleLinesStepDef extends BaseStep {
	...
	public BusinessBaseAddOnPromotionsDoubleLinesStepDef() throws IOException {
		super();
	}
	...
	@Given("^DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) and Double Order Line (\\d+) in (.*) to be Utilised to Get Offer for Business Base AddOn Member$")
	public void createthesingleLineRequestBusinessBaseAddOn(String channelName, String ClubId, String ClubId2, int code,
		...
		while (i < size) {
			...
			UpdatetheRequesttoDoubleLinesBusinessBaseAddOn(ClubId, ClubId2, RetailPrice, lineNumber2, Applied_Dates);

			POST_the_request_for_Double_linesBusinessBaseAddOn();
			Getparameter_and_postOperation_for_Double_LinesBusinessBaseAddOn();

			POST_the_emulator_request_for_Double_linesBusinessBaseAddOn();
			Getemulatorparameter_and_postOperation_for_Double_linesBusinessBaseAddOn();

			ChecktheResultsdoubleLinesBusinessBaseAddOn();
	...
	@Given("^DataPower with Second Item and (\\d+),(\\d+) with (\\d+) and Second OrderLine (\\d+) in (.*) to be Utilised to Get Offer for Business Base AddOn Member$")

	public void UpdatetheRequesttoDoubleLinesBusinessBaseAddOn(String ClubId, String ClubId2, int RetailPrice,
			String lineNumber, String Applied_Dates) throws Exception {

		postDoubleRequestDetails=addOnprimaryrequest.getBusinessBaseAddOnPostRequestDetails(disc, disc2, packagecode, 
	...
	@When("^DataPower Endpoint POST request for Business Base AddOn Member$")
	public void POST_the_request_for_Double_linesBusinessBaseAddOn() {
	...
	@When("^Post the request to Emulator Endpoint for Business Base AddOn Member$")
	public void POST_the_emulator_request_for_Double_linesBusinessBaseAddOn() {
	...
	@When("^Response for DataPower and get the required discount for Business Base AddOn Member$")
	public void Getparameter_and_postOperation_for_Double_LinesBusinessBaseAddOn() {

		response = thisRequestSpecification.post(UrlConstants.SERVICES_CHECKOUT).then();

	}

	@When("^Response for Emulator and get the required discount for Business Base AddOn Member$")
	public void Getemulatorparameter_and_postOperation_for_Double_linesBusinessBaseAddOn() {

		response2 = thisRequestSpecification2.post(UrlConstants.RESERVE_EMULATION).then();

	}

	@Then("^Assert on DataPower request and get attributes for Business Base AddOn Member$")
	public void ChecktheResultsdoubleLinesBusinessBaseAddOn() throws Exception {

		softAssertions=asserthelper.BusinessBaseAddOnDoubleLinesValidation(response, response2, packagecode, packagecode2, OfferId, OfferId2, expected);
