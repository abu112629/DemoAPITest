...
public class SavingsBaseAddOnComplimentaryPromotionsDoubleLinesStepDef extends BaseStep {
	...
	public SavingsBaseAddOnComplimentaryPromotionsDoubleLinesStepDef() throws IOException {
		super();
	}
	...
	@Given("^DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) and Double Order Line (\\d+) in (.*) to be Utilised to Get Offer for AddOn Complimentary Member$")
	public void createthesingleLineRequestAddOnComplimentary(String channelName, String ClubId, String ClubId2, int code, int RetailPrice,
		...
		while (i < size) {
			...
			UpdatetheRequesttoDoubleLinesAddOnComplimentary(ClubId, ClubId2, RetailPrice, lineNumber2, Applied_Dates);

			POST_the_request_for_Double_linesAddOnComplimentary();
			Getparameter_and_postOperation_for_Double_LinesAddOnComplimentary();

			POST_the_emulator_request_for_Double_linesAddOnComplimentary();
			Getemulatorparameter_and_postOperation_for_Double_linesAddOnComplimentary();

			ChecktheResultsdoubleLinesAddOnComplimentary();
	...
	@Given("^DataPower with Second Item and (\\d+),(\\d+) with (\\d+) and Second OrderLine (\\d+) in (.*) to be Utilised to Get Offer for AddOn Complimentary Member$")

	public void UpdatetheRequesttoDoubleLinesAddOnComplimentary(String ClubId, String ClubId2, int RetailPrice, String lineNumber,
	...
	@When("^DataPower Endpoint POST request for AddOn Complimentary Member$")
	public void POST_the_request_for_Double_linesAddOnComplimentary() {
	...
	@When("^Post the request to Emulator Endpoint for AddOn Complimentary Member$")
	public void POST_the_emulator_request_for_Double_linesAddOnComplimentary() {
	...
	@When("^Response for DataPower and get the required discount for AddOn Complimentary Member$")
	public void Getparameter_and_postOperation_for_Double_LinesAddOnComplimentary() {

		response = thisRequestSpecification.post(UrlConstants.SERVICES_CHECKOUT).then();

	}

	@When("^Response for Emulator and get the required discount for AddOn Complimentary Member$")
	public void Getemulatorparameter_and_postOperation_for_Double_linesAddOnComplimentary() {

		response2 = thisRequestSpecification2.post(UrlConstants.RESERVE_EMULATION).then();

	}

	@Then("^Assert on DataPower request and get attributes for AddOn Complimentary Member$")
	public void ChecktheResultsdoubleLinesAddOnComplimentary() throws Exception {
