...
public class BusinessPlusComplimentaryPromotionsStepDef extends BaseStep {
	...
	public BusinessPlusComplimentaryPromotionsStepDef() throws IOException {
		super();
	}
	...
	@Given("^Minimum required for DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) in (.*) to be Utilised to Get Offer for Business Complimentary Plus Member$")
	public void createtheRequestBusinessComplimentaryPlus(String channelName, String ClubId, String ClubId2, int code,
		...
		while (i < size) {
			...
			POST_the_requestBusinessComplimentaryPlus();
			Getparameter_and_postOperationBusinessComplimentaryPlus();

			POST_the_emulator_requestBusinessComplimentaryPlus();
			Getemulatorparameter_and_postOperationBusinessComplimentaryPlus();

			ChecktheResultsBusinessComplimentaryPlus();
	...
	@When("^Post request to Emulator Endpoint for Business Complimentary Plus Member$")
	public void POST_the_emulator_requestBusinessComplimentaryPlus() {
	...
	@When("^Request posted to DataPower and get the required discount for Business Complimentary Plus Member$")
	public void Getparameter_and_postOperationBusinessComplimentaryPlus() {

		response = thisRequestSpecification.post(UrlConstants.SERVICES_CHECKOUT).then();

	}

	@When("^Request posted to Emulator and get the required discount for Business Complimentary Plus Member$")
	public void Getemulatorparameter_and_postOperationBusinessComplimentaryPlus() {

		response2 = thisRequestSpecification2.post(UrlConstants.RESERVE_EMULATION).then();

	}

	@Then("^Assert required to DataPower and get attributes for Business Complimentary Plus Member$")
	public void ChecktheResultsBusinessComplimentaryPlus() throws Exception {

		softAssertions=asserthelper.BusinessPlusValidation(response, response2, packagecode, OfferId, expected);
