...
public class SavingsPlusComplimentaryPromotionsStepDef extends BaseStep {
	...
	public SavingsPlusComplimentaryPromotionsStepDef() throws IOException {
		super();
	}
	...
	@Given("^Minimum required for DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) in (.*) to be Utilised to Get Offer for Savings Complimentary Plus Member$")
	public void createtheRequestSavingsPlusComplimentary(String channelName, String ClubId, String ClubId2, int code,
		...
		while (i < size) {
			...
			POST_the_requestSavingsPlusComplimentary();
			Getparameter_and_postOperationSavingsPlusComplimentary();

			POST_the_emulator_requestSavingsPlusComplimentary();
			Getemulatorparameter_and_postOperationSavingsPlusComplimentary();

			ChecktheResultsSavingsPlusComplimentary();
	...
	@When("^Post request to DataPower Endpoint for Savings Complimentary Plus Member$")
	public void POST_the_requestSavingsPlusComplimentary() {
	...
	@When("^Post request to Emulator Endpoint for Savings Complimentary Plus Member$")
	public void POST_the_emulator_requestSavingsPlusComplimentary() {
	...
	@When("^Request posted to DataPower and get the required discount for Savings Complimentary Plus Member$")
	public void Getparameter_and_postOperationSavingsPlusComplimentary() {

		response = thisRequestSpecification.post(UrlConstants.SERVICES_CHECKOUT).then();

	}

	@When("^Request posted to Emulator and get the required discount for Savings Complimentary Plus Member$")
	public void Getemulatorparameter_and_postOperationSavingsPlusComplimentary() {

		response2 = thisRequestSpecification2.post(UrlConstants.RESERVE_EMULATION).then();

	}

	@Then("^Assert required to DataPower and get attributes for Savings Complimentary Plus Member$")
	public void ChecktheResultsSavingsPlusComplimentary() throws Exception {

		softAssertions=asserthelper.SavingsPlusValidation(response, response2, packagecode, OfferId, expected);
