...
public class SavingsBaseAddOnPromotionsStepDef extends BaseStep {
	...
	public SavingsBaseAddOnPromotionsStepDef() throws IOException {
		super();
	}
	...
	@Given("^Minimum required for DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) in (.*) to be Utilised to Get Offer for AddOn Member$")
	public void createtheRequestAddOn(String channelName, String ClubId, String ClubId2, int code, int RetailPrice,
		...
		while (i < size) {
			...
			POST_the_requestAddOn();
			Getparameter_and_postOperationAddOn();

			POST_the_emulator_requestAddOn();
			Getemulatorparameter_and_postOperationAddOn();

			ChecktheResultsAddOn();
	...
	@When("^Post request to DataPower Endpoint for AddOn Savings Member$")
	public void POST_the_requestAddOn() {
	...
	@When("^Post request to Emulator Endpoint for AddOn Savings Member$")
	public void POST_the_emulator_requestAddOn() {
	...
	@When("^Request posted to DataPower and get the required discount for AddOn Savings Member$")
	public void Getparameter_and_postOperationAddOn() {

		response = thisRequestSpecification.post(UrlConstants.SERVICES_CHECKOUT).then();

	}

	@When("^Request posted to Emulator and get the required discount for AddOn Savings Member$")
	public void Getemulatorparameter_and_postOperationAddOn() {

		response2 = thisRequestSpecification2.post(UrlConstants.RESERVE_EMULATION).then();

	}

	@Then("^Assert required to DataPower and get attributes for AddOn Savings Member$")
	public void ChecktheResultsAddOn() throws Exception {
