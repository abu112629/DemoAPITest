...
public class BusinessPlusAddOnComplimentaryPromotionsDoubleLinesStepDef extends BaseStep {
	...
	public BusinessPlusAddOnComplimentaryPromotionsDoubleLinesStepDef() throws IOException {
		super();
	}
	...
	@Given("^DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) and Double Order Line (\\d+) in (.*) to be Utilised to Get Offer for Business Plus AddOn Complimentary Member$")
	public void createthesingleLineRequestBusinessPlusAddOnComplimentary(String channelName, String ClubId, String ClubId2, int code,
		...
		while (i < size) {
			...
			UpdatetheRequesttoDoubleLinesBusinessPlusAddOnComplimentary(ClubId, ClubId2, RetailPrice, lineNumber2, Applied_Dates);

			POST_the_request_for_Double_linesBusinessPlusAddOnComplimentary();
			Getparameter_and_postOperation_for_Double_LinesBusinessPlusAddOnComplimentary();

			POST_the_emulator_request_for_Double_linesBusinessPlusAddOnComplimentary();
			Getemulatorparameter_and_postOperation_for_Double_linesBusinessPlusAddOnComplimentary();

			ChecktheResultsdoubleLinesBusinessPlusAddOnComplimentary();
	...
	@Given("^DataPower with Second Item and (\\d+),(\\d+) with (\\d+) and Second OrderLine (\\d+) in (.*) to be Utilised to Get Offer for Business Plus AddOn Complimentary Member$")

	public void UpdatetheRequesttoDoubleLinesBusinessPlusAddOnComplimentary(String ClubId, String ClubId2, int RetailPrice,
			String lineNumber, String Applied_Dates) throws Exception {

		postDoubleRequestDetails=addOnprimaryrequest.getBusinessPlusAddOnPostRequestDetails(disc, disc2, packagecode, 
	...
	@When("^DataPower Endpoint POST request for Business Plus AddOn Complimentary Member$")
	public void POST_the_request_for_Double_linesBusinessPlusAddOnComplimentary() {
	...
	@When("^Post the request to Emulator Endpoint for Business Plus AddOn Complimentary Member$")
	public void POST_the_emulator_request_for_Double_linesBusinessPlusAddOnComplimentary() {
	...
	@When("^Response for DataPower and get the required discount for Business Plus AddOn Complimentary Member$")
	public void Getparameter_and_postOperation_for_Double_LinesBusinessPlusAddOnComplimentary() {

		response = thisRequestSpecification.post(UrlConstants.SERVICES_CHECKOUT).then();

	}

	@When("^Response for Emulator and get the required discount for Business Plus AddOn Complimentary Member$")
	public void Getemulatorparameter_and_postOperation_for_Double_linesBusinessPlusAddOnComplimentary() {

		response2 = thisRequestSpecification2.post(UrlConstants.RESERVE_EMULATION).then();

	}

	@Then("^Assert on DataPower request and get attributes for Business Plus AddOn Complimentary Member$")
	public void ChecktheResultsdoubleLinesBusinessPlusAddOnComplimentary() throws Exception {

		softAssertions=asserthelper.BusinessPlusAddOnDoubleLinesValidation(response, response2, packagecode, packagecode2, OfferId, OfferId2, expected);
