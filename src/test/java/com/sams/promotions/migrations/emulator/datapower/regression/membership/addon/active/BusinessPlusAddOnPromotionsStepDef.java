...
public class BusinessPlusAddOnPromotionsStepDef extends BaseStep {
	...
	public BusinessPlusAddOnPromotionsStepDef() throws IOException {
		super();
	}
	...
	@Given("^Minimum required for DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) in (.*) to be Utilised to Get Offer for Business AddOn Plus Member$")
	public void createtheRequestBusinessPlusAddOn(String channelName, String ClubId, String ClubId2, int code,
		...
		while (i < size) {
			...
			POST_the_requestBusinessPlusAddOn();
			Getparameter_and_postOperationBusinessPlusAddOn();

			POST_the_emulator_requestBusinessPlusAddOn();
			Getemulatorparameter_and_postOperationBusinessPlusAddOn();

			ChecktheResultsBusinessPlusAddOn();
	...
	@When("^Post request to DataPower Endpoint for Business AddOn Plus Member$")
	public void POST_the_requestBusinessPlusAddOn() {
	...
	@When("^Post request to Emulator Endpoint for Business AddOn Plus Member$")
	public void POST_the_emulator_requestBusinessPlusAddOn() {
	...
	@When("^Request posted to DataPower and get the required discount for Business AddOn Plus Member$")
	public void Getparameter_and_postOperationBusinessPlusAddOn() {

		response = thisRequestSpecification.post(UrlConstants.SERVICES_CHECKOUT).then();

	}

	@When("^Request posted to Emulator and get the required discount for Business AddOn Plus Member$")
	public void Getemulatorparameter_and_postOperationBusinessPlusAddOn() {

		response2 = thisRequestSpecification2.post(UrlConstants.RESERVE_EMULATION).then();

	}

	@Then("^Assert required to DataPower and get attributes for Business AddOn Plus Member$")
	public void ChecktheResultsBusinessPlusAddOn() throws Exception {

		softAssertions=asserthelper.BusinessPlusAddOnValidation(response, response2, packagecode, OfferId, expected);
