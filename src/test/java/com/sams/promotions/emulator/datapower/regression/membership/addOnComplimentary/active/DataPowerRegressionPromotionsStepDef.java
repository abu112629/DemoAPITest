package com.sams.promotions.emulator.datapower.regression.membership.addOnComplimentary.active;
...
public class DataPowerRegressionPromotionsStepDef extends BaseStep {
	...
	private Response membershipResponse, add_On_memberhipResponse, add_On_Complimentary_memberhipResponse;
	...
	@Given("^Minimum (\\d+) required for DataPower with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) in (.*) to be Utilised to Get Offer (.*) for AddOn Complimentary Member$")
	public void createtheRequest(int Quantity, String channelName, int ItemId, String ClubId, int code, int RetailPrice,
		...
		JSONObject req2 = helper.updateMembershipRequest("ADD_ON", membershipNbr,
		...
		add_On_membershipNbr = helper.getResponseValue(add_On_memberhipResponse,
				MembershipConstants.MEMBERSHIP_MEMBERSHIPCARD_PATH);

		JSONObject req3 = helper.updateMembershipRequest("ADD_ON_COMPLIMENTARY", add_On_membershipNbr,
				MembershipConstants.MEMBERSHIP_SAVINGS_BASE_REQUEST_PATH);
		add_On_Complimentary_memberhipResponse = membership.createOtherMembership(req3.toJSONString());
		add_On_Complimentary_membershipNbr = helper.getResponseValue(add_On_Complimentary_memberhipResponse,
				MembershipConstants.MEMBERSHIP_MEMBERSHIPCARD_PATH);
		  ...
		  switch(Applied_Dates)
		  {
		  	case "LAST_DATE":
		  		postdata = reserveemulator.XMLRequestUpdater(Quantity, ItemId, RetailPrice, ClubId, lineNumber, code,
						channelName,add_On_Complimentary_membershipNbr,map.get("lastdate"),pathsingle);
		  		break;
		  	case "FIRST_DATE":
		  		postdata = reserveemulator.XMLRequestUpdater(Quantity, ItemId, RetailPrice, ClubId, lineNumber, code,
						channelName,add_On_Complimentary_membershipNbr,map.get("firstdate"),pathsingle);
		  		break;
		  	case "MIDDLE_DATE":	
		  		postdata = reserveemulator.XMLRequestUpdater(Quantity, ItemId, RetailPrice, ClubId, lineNumber, code,
						channelName,add_On_Complimentary_membershipNbr,map.get("midDate"),pathsingle);
		  		break;
	...
	@Given("^Minimum (\\d+) required for DataPower with Second (\\d+) and (\\d+) with (\\d+) and Second OrderLine (\\d+) to be Utilised to Get Offer for AddOn Complimentary Member$")
	...
	@When("^Post request to DataPower Endpoint for AddOn Complimentary Member$")
	...
	@When("^Request posted to DataPower and get the required discount for AddOn Complimentary Member$")
	...
	@Then("^Assert required to DataPower and get attributes for AddOn Complimentary Member$")
