...
public class BroadreachMemberTypesStepDef extends BaseStep {
	...
	@Given("DataPower with a (.*), (.*), (.*), and (.*) with (\\d+) for InstantSavings and (\\d+) for QuickSilver with Code (\\d+) and Price (\\d+) and Single OrderLine (\\d+), Second Order Line (\\d+) in (.*) with Registration Number (\\d+) and TransactionId (\\d+) to be Utilised to Get Offer$")
	public void createtheInitialLinerequestforAllMembers(String membershipBase, String type, String Tier,
		...
		String membershipNbr = member.getMembershipType(type, membershipBase, Tier);
	...
	@Given("^DataPower with a (.*),(.*),(.*), and (.*) with (\\d+) for InstantSavings and (\\d+) for QuickSilver with code (\\d+) and Price (\\d+) and Single OrderLine (\\d+) in (.*) with Registration Number (\\d+) and TransactionId (\\d+) to be Utilised to Get Offer$")
	...
	public void createtheSingleLineRequest(String membershipBase,String type,String Tier,
		...
		while (i < 1) {
			...
			postRequestDetails = reserveemulator.getReserveRequestDetails(i, membershipNbr, channelName, ClubId,
					ClubId2, code, RetailPrice, lineNumber, Applied_Dates, OfferId, RegistrationNumber, TransactionId,
					pathsingle);
	...
	// @Given("^DataPower with Second Item and (\\d+) for InstantSavings, (\\d+) for
	// QuickSilver with Price (\\d+) and Second OrderLine (\\d+) in (.*) to be
	// Utilised to Get Offer$")
