...
public class EmulatorRegressionPromotionsStepDef extends BaseStep {
	...
	@Given("^Minimum (\\d+) required for Emulator with (.*),(\\d+),(\\d+) and (\\d+) with (\\d+) and Single OrderLine (\\d+) in (.*) to be Utilised to Get Offer (.*) for complimentary member$")
	public void createComplimentaryRequest(int Quantity, String channelName, int ItemId, String ClubId, int code,
	...
	@Given("^Minimum (\\d+) required for Emulator with Second (\\d+) and (\\d+) with (\\d+) and Second OrderLine (\\d+) to be Utilised to Get Offer for Complimentary Member$")
	public void UpdateComplimentaryRequest(int Quantity, int ItemId2, String ClubId, int RetailPrice, String lineNumber)
			throws Exception {
	...
	public void POST_Complimentaryrequest() {
  
		thisRequestSpecification.given().relaxedHTTPSValidation("TLS").header("Content-Type", "text/xml").body(postdata) .when(); 
		//header("Content-Type","text/xml"). 
}

	@When("^Request posted to Emulator and get the required discount for Complimentary Member$")
	public void GetComplimentaryparameter_postOperation() {
	...
	public void CheckComplimentaryResults(DataTable dataTable) throws Exception {
